...
bool GUI::LoadAssets(wxString& error, wxArrayString& warnings, bool force)
{
...
bool GUI::LoadDataFiles(wxString& error, wxArrayString& warnings)
{
	...
	FileName assets_path = FileName(g_settings.getString(Config::ASSETS_DIRECTORY) + FileName::GetPathSeparator());
	...
	if(!g_gui.gfx.loadSpriteMetadata(wxString(assets_path.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR) + (wxString(ASSETS_NAME) + ".dat")), error, warnings)) {
	...
	if(!g_gui.gfx.loadSpriteData(wxString(assets_path.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR) + (wxString(ASSETS_NAME) + ".spr")), error, warnings)) {
