	...
	replace_brush(nullptr)
{
	...
	if(!g_gui.LoadAssets(error, warnings))
		throw std::runtime_error("Couldn't load assets");
...
{
	MapVersionID ver;
	if(!IOMapRBM::getVersionInfo(fn, ver)) {
	...
	wxString error;
	wxArrayString warnings;
	success = g_gui.LoadAssets(error, warnings);
	if(!success)
...
void Editor::saveMap(FileName filename, bool showdialog)
{
	...
	//converter.Assign(wxstr(savefile));
	std::string backup_rbm, backup_house, backup_spawn;

	if(converter.GetExt() == "rgz") {
		save_otgz = true;
		if(converter.FileExists()) {
			backup_rbm = map_path + nstr(converter.GetName()) + ".rgz~";
			std::remove(backup_rbm.c_str());
			std::rename(savefile.c_str(), backup_rbm.c_str());
		}
	} else {
		if(converter.FileExists()) {
			backup_rbm = map_path + nstr(converter.GetName()) + ".rbm~";
			std::remove(backup_rbm.c_str());
			std::rename(savefile.c_str(), backup_rbm.c_str());
		}
...

	{
		...
		IOMapRBM mapsaver(map.getVersion());
		...
		if(!success) {
			...
			if(!backup_rbm.empty()) {
				...
				std::string rbm_filename = map_path + nstr(converter.GetName());
				std::rename(backup_rbm.c_str(), std::string(rbm_filename + (save_otgz ? ".rgz" : ".rbm")).c_str());
			}
	...
	if(!save_as && g_settings.getInteger(Config::ALWAYS_MAKE_BACKUP)) {
		...
		if(!backup_rbm.empty()) {
			converter.SetFullName(wxstr(savefile));
			std::string rbm_filename = map_path + nstr(converter.GetName());
			std::rename(backup_rbm.c_str(), std::string(rbm_filename + "." + date.str() + (save_otgz ? ".rgz" : ".rbm")).c_str());
		}
