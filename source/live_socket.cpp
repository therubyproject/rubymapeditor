...
LiveSocket::LiveSocket() :
	...
	mapVersion(MapVersionID(MAP_RBM_1)), log(nullptr),
	name("User"), password("")
{
	//
}
...
{
	writer.addNode(tile->isHouseTile() ? RBM_HOUSETILE : RBM_TILE);
	if(position) {
	...
	if(tile->getMapFlags()) {
		writer.addByte(RBM_ATTR_TILE_FLAGS);
	...
	if(ground) {
		if(ground->isComplex()) {
			ground->serializeItemNode_RBM(mapVersion, writer);
		} else {
			writer.addByte(RBM_ATTR_ITEM);
			ground->serializeItemCompact_RBM(mapVersion, writer);
		}
	}

	for(Item* item : tile->items) {
		item->serializeItemNode_RBM(mapVersion, writer);
	}
...
Tile* LiveSocket::readTile(BinaryNode* node, Editor& editor, const Position* position)
{
	...
	if(tileType != RBM_TILE && tileType != RBM_HOUSETILE) {
		return nullptr;
	}
	...
	while(node->getU8(attribute)) {
		switch (attribute) {
			...
			case RBM_ATTR_ITEM: {
				Item* item = Item::Create_RBM(mapVersion, node);
				if(!item) {
	...
	if(itemNode) do {
		...
		if(itemType == RBM_ITEM) {
			Item* item = Item::Create_RBM(mapVersion, itemNode);
			if(item) {
				if(!item->unserializeItemNode_RBM(mapVersion, itemNode)) {
