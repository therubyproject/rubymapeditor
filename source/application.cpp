...
bool Application::OnInit()
{
	...
	g_settings.load();
	FixVersionDiscrapencies(); // maybe change the name?
	g_gui.LoadHotkeys();
	...
	if(g_settings.getInteger(Config::GOTO_WEBSITE_ON_BOOT) == 1) {
		::wxLaunchDefaultBrowser("https://github.com/therubyproject", wxBROWSER_NEW_WINDOW);
	...
	if(save_failed_file.FileExists()) {
		...
		if(!backup_rbm.empty()) {
			...
			if(ret == wxID_YES) {
				...
				std::remove(backup_rbm.substr(0, backup_rbm.size() - 1).c_str());
				std::rename(backup_rbm.c_str(), backup_rbm.substr(0, backup_rbm.size() - 1).c_str());
				...
				// Load the map
				g_gui.LoadMap(wxstr(backup_rbm.substr(0, backup_rbm.size() - 1)));
				return true;
			}
		}
	}
