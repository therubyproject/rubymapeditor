	...
	only_pickupables(onlyPickupables)
{
	...
	wxString radio_boxChoices[] = { "Find by ID",
									"Find by Name",
									"Find by Types",
									"Find by Properties" };
	...
	options_radio_box->SetSelection(SearchMode::IDs);
	options_box_sizer->Add(options_radio_box, 0, wxALL | wxEXPAND, 5);

	wxStaticBoxSizer* server_id_box_sizer = newd wxStaticBoxSizer(newd wxStaticBox(this, wxID_ANY, "ID"), wxVERTICAL);
...
void FindItemDialog::setSearchMode(FindItemDialog::SearchMode mode)
{
	...
	server_id_spin->Enable(mode == SearchMode::IDs);
